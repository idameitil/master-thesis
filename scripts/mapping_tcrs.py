...
vdjdb_file = open("/home/ida/master-thesis/VDJdb_minscore0_2021-02-11.tsv", "r")
TRB_cdrs = dict()
TRA_cdrs = dict()
wrong_start = 0
wrong_end = 0
for line in vdjdb_file:
    ...
    J = splitted_line[4]
    if gene == "TRB":
        TRB_cdrs[cdr3_trimmed] = [V, J, cdr3]
    elif gene == "TRA":
        TRA_cdrs[cdr3_trimmed] = [V, J, cdr3]
vdjdb_file.close()
#print("Wrong start: {}. Wrong end: {}.". format(wrong_start, wrong_end))

train_file = open("/home/ida/master-thesis/tcr_data/train_ab95_5x_umi0.csv", "r")
outfile = open("/home/ida/master-thesis/positives.csv", "w")
outfile.write("#CDR3a,CDR3b,peptide,partition,binder,v_gene_alpha,j_gene_alpha,v_gene_beta,j_gene_beta\n")
...
for line in train_file:
    ...
    beta_succeded = False
    try:
        v_gene_beta, j_gene_beta = TRB_cdrs[cdr3b]
        beta_succeded = True
    except KeyError:
        pass
    # If previous fails, remove W if there
    if not beta_succeded:
        if cdr3b.endswith("W") or cdr3.endswith("F"):
            cdr3b = cdr3b[:-1]
    ...
    try:
        v_gene_alpha, j_gene_alpha = TRA_cdrs[cdr3a]
        alpha_succeded = True
    except KeyError:
        pass
